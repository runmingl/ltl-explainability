...
# 
# Output: an LTL formula of the form C1(n) = || _{i=1} ^{n} G F pi
#
# Usage: C1formula.pl n
#
...
if (@ARGV != 1) {
    die "Usage: C1formula.pl n\n\tproduces an n-part formula of the form C1(n) = || _{i=1} ^{n} G F pi\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
