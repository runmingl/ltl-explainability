...
{\bf Scalability.}
We study the scalability of our tool on a benchmark suite of LTL formulas that scales in size. According to Rozier and Vardi~\cite{RV10}, the LTL formula required to encode a $n$-bit binary counter scales in size as $n$ gets larger. Therefore the benchmark for scalability consists of LTL formulas that describe $n$-bit counters. \cref{fig:scalability} shows that the timeline length grows approximately exponentially with the number of bits in the counter, which matches the result in Rozier and Vardi~\cite{RV10}.
...
\]
which comes with two command-line tools, \texttt{ltl2regex} and \tool. The specific usage of the tool can be found in the artifact repository.

The set of LTL formulas we used to evaluate our tool in \cref{sec:playing} can be found in the artifact repository under \texttt{ltl-formulas} directory.
