...
In the following section, we prove the correctness of our translation pipeline. The currectness of the translation from LTL formulas to \Buchi automata is provided by SPOT~\cite{Dur22}. Below, we prove the correctness of (i) the translation from \Buchi automata to $\omega$-regular expressions, (ii) the ($\omega$-)regular expression rewrite rules we apply, and lastly, we outline the correctness of our timeline visualizations. 
...
\begin{enumerate}
  ...
  \item $B = AB$. Visualized as the timeline of $A$ followed by the timeline of $B$. A concatenation of a regular expression and an $\omega$-regular expression signifies that the first must be true before the second $\omega$-regular expression can be true in the following timesteps.
