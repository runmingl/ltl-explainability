...
%KYR: FIX THIS! We need to make it clear the relationship between Algorithms 1..4 and the figure displaying the high-level algorithm.
The algorithm to convert \Buchi automata to $\omega$-regular expressions \texttt{ba2wregex} combines Algorthms 1--4 below. %\cref{alg:ba2wregex}
...
\begin{proof}
    ...
    Also, we show $\L' \subseteq \L(G')$, i.e., every path starting in $s$ and ending by reaching $f$ for the first time
...
\begin{proof}
    We show that
    \[
        \L = \bigcup_{f\in F} \textbf{nfa2regex}(G, s, f)  \left(\textbf{nfa2regex\_firstvisit}(G, f, f)\right)^\omega
    \]
    and the $\omega$-language accepted by $G$, $\L_\omega(G)$ are identical.

    First, $\L\subseteq \L_\omega(G)$ since for every infinite run in $\L$, by definition of $\L$, there is some $f\in F$ such that passes through $f$ infinitely often. The converse is true as well: for every infinite run $q_0, q_1, \dots$ accepted by $G$, then $q_0 = s$ and by definition of the accepting language, there is some accepting state $f$, passed through infinitely often; thus the run is in $\textbf{nfa2regex}(G, s, f)$ $\left(\textbf{nfa2regex\_firstvisit}(G, f, f)\right)^\omega$.
\end{proof}
...
\begin{definition}[Timeline Length]
    ...
    \begin{align*}
        \tllen(\emptyset)   & \quad \text{undefined} \\
        \tllen(\epsilon)    &= 0\\
        \tllen(a)           &= 1\\
...
%
We gather a test suite totalling 91 formulas from two real-world requirement specification applications. We take $6$ specifications from NASA's Automated Airspace Concept (AAC)~\cite{GCMTR16}. We also collect formulas from the Acacia suite of examples~\cite{RV10}: $85$ formulas from the suite of $23$ collections of formulas were collected. We choose to visualize each formula individually, rather than visualize a complete set of (conjuncted) specifications, as a complete specification set may describe a large, complex system, and we only target the use case of validating individual formulas. The set of formulas being tested can be found \href{https://github.com/EULIR/ltl-explainability/blob/main/ltl-formulas/benchmark-ltls.txt}{here}.
...
{\bf Scalability.}
%KYR: FIX!!! The description of the exact experiment run is not clear and seems to contradict the points in the above paragraph...
We measure the scalability of our tool by running it against a set of \emph{scalable} LTL formulas. We define a scalable LTL formula as one that describes an $n$-bit number using a binary counter, as generated by~\cite{RV10}. As $n$ gets larger, the LTL formula required to encode the $n$-bit counter scales in size. \cref{fig:scalability} shows that the timeline length grows approximately exponentially with the number of bits in the counter, which matches the result in Rozier and Vardi~\cite{RV10}. The set of formulas being tested can be found \href{https://github.com/EULIR/ltl-explainability/tree/main/ltl-formulas/binary_counter}{here}.

In \cref{fig:scalability}, the $x$-axis ranges from $1$ to $6$, after which point it hits the $30$ seconds timeout we set for the scalability tests, as the timeline length grows exponentially fast.

%KYR: FIX: is timeout 20 seconds or 30 seconds?!?!? It still sounds like this is the same test as the paragraph above but with a different timeout. Please describe the difference in the tests; what was run exactly for a 30 second timeout and why was this different than the 20 second timout?
