...
\begin{example}
    The formula $\Phi = \always (a \limplies \eventually (\neg a))$ generates un-simplified $\omega$-regular expression $((!a) | ((aa^*(!a)))((!a) | ((aa^*(!a)))^{\omega}$, and simplified version $((!a) | ((aa^*(!a)))^{\omega}$, which corresponds to the two timelines in figure \cref{fig:unsimplified} and \cref{fig:simplified}.
    \begin{figure}[h!]
        \centering
